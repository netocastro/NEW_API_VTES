...
<div class="d-flex justify-content-center">
    <h2 class="text-center my-3">Have list: <?= $name ?></h2>
        ...
        <div class="col-10 col-md-8 col-sm-10">
            <div class="row no-spaces">
                ...
                <div class="col-md-2 col-sm-2">
                    <button type="button" class="btn btn-outline-success btn-sm ms-1" id="save-have-list" data-url="<?= $route->route('request.saveHaveList') ?>">Update</button>
                </div>
            </div>
        </div>
        <div class="col-1 col-md-2 col-sm-1"></div>
    ...
    <div class="col-12 col-md-8 mt-3">
        <ul class="nav nav-pills nav-fill mb-3 justify-content-center" id="pills-tab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="pills-home-tab" data-bs-toggle="pill" data-bs-target="#pills-home" type="button" role="tab" aria-controls="pills-home" aria-selected="true">Crypt<span id="total-have-crypt"></span></button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="pills-profile-tab" data-bs-toggle="pill" data-bs-target="#pills-profile" type="button" role="tab" aria-controls="pills-profile" aria-selected="false">Library<span id="total-have-library"></span></button>
            </li>
        </ul>
        <div class="tab-content" id="pills-tabContent">
            <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                <table class="table table-sm">
                    ...
                    <tbody id="cryptList">
                        <?php if ($haveListCrypt) : ?>
                            <?php foreach ($haveListCrypt as $haveCrypt) : ?>
                                <tr data-id="<?= $haveCrypt->cryptCard()->id ?>">
                                    <td class="amount-crypt"><?= $haveCrypt->amount ?></td>
                                    <td class="card-name">
                                        <div class="d-grid">
                                            <button class="btn btn-link text-start" data-bs-toggle="modal" data-bs-target="#exampleModal" data-src=<?= $haveCrypt->cryptCard()->cardName(); ?>><?= trim($haveCrypt->cryptCard()->name); ?></button>

                                        </div>
                                    </td>
                                    <td><?= $haveCrypt->cryptCard()->clan ?></td>
            ...
            <div class="tab-pane fade" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab">
                <table class="table table-sm">
                    ...
                    <tbody id="libraryList">
                        <?php if ($haveListLibrary) : ?>
                            <?php foreach ($haveListLibrary as $haveLibrary) : ?>
                                <tr data-id="<?= $haveLibrary->libraryCard()->id ?>">
                                    <td class="amount-library"><?= $haveLibrary->amount ?></td>
                                    <td class="card-name">
                                        <div class="d-grid">
                                            <button class="btn btn-link text-start" data-bs-toggle="modal" data-bs-target="#exampleModal" data-src=<?= $haveLibrary->libraryCard()->cardName(); ?>><?= trim($haveLibrary->libraryCard()->name); ?></button>
                                        </div>
                                    </td>
                                    <td><?= $haveLibrary->libraryCard()->type ?></td>
                                    <?php if (isset($_SESSION['user_id']) && $user->id == $_SESSION['user_id']) : ?>
                                        <td class="text-center">
                                            <div class=" btn-group">
...
<script src="<?= url('view/js/haveList.js'); ?>"></script>
